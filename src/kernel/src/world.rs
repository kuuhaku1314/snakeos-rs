...
impl World {
    ...
    pub fn on_keypress(&mut self, key: DecodedKey, display: &mut Display) {
        match self.state {
            State::Running(_) if key == DecodedKey::Unicode('r') => {
    ...
    fn draw_game_over(&mut self, display: &mut Display) {
        ...
        let msg = "GAME OVER";
        display.set_xy(
            display.info.unwrap().width / 2 - msg.len() * 8 / 2,
            display.info.unwrap().height / 2,
        );
        display.write_str(msg).unwrap();
        let msg = "(press 'r' to restart)";
        display.set_xy(
            display.info.unwrap().width / 2 - msg.len() * 8 / 2,
            display.info.unwrap().height / 2 + 10,
        );
    ...
    pub fn draw_welcome(&mut self, display: &mut Display) {
        let (w, h) = (
            display.info.unwrap().width,
            display.info.unwrap().height,
        );
        display.clear();
